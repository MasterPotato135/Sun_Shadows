//main/java/com/limelight/Game.java
...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Only enable the 4G/Tailscale optimization branch on real mobile (cellular) links,
        // or as a fallback, on networks the OS reports as metered. Checking transport type
        // directly (TRANSPORT_CELLULAR) is what actually identifies "am I on 4G/5G" -
        // isActiveNetworkMetered() alone is not reliable: some carriers don't mark cellular
        // as metered (optimizations would never activate on 4G), and some Wi-Fi networks are
        // marked metered even though they're not a mobile radio link. This keeps the legacy
        // stable behavior on normal LAN/Wi-Fi connections while making sure the optimizations
        // actually trigger on real 4G/5G connections.
        boolean useNetworkOptimizations = prefConfig.mobileNetworkOptimizations &&
                (NetHelper.isActiveNetworkCellular(this) || connMgr.isActiveNetworkMetered());

        // Check if the user has enabled performance stats overlay.
        // On metered mobile paths the overlay adds extra work to an already stressed decode/render
        // loop, so keep it hidden unless the user is on a stable local connection.
        boolean showPerfOverlay = prefConfig.enablePerfOverlay && !useNetworkOptimizations;
        if (showPerfOverlay) {
        ...
        int streamBitrate = prefConfig.bitrate;
        int streamFps = prefConfig.fps;

        if (useNetworkOptimizations) {
            // On restricted connections, avoid pushing high-res frames over slow links.
            // The goal here is to keep the stream alive rather than to maximize the screen size.
            if (streamWidth > 1920 || streamHeight > 1080) {
                streamWidth = 1920;
                streamHeight = 1080;
                LimeLog.info("Slow network profile: reducing stream to 1080p for stability");
            } else if (streamWidth > 1280 || streamHeight > 720) {
                streamWidth = 1280;
                streamHeight = 720;
                LimeLog.info("Slow network profile: reducing stream to 720p for stability");
            }
            ...
            // Only cap FPS when the bitrate is also being constrained. On good 4G/5G links
            // where the user's chosen bitrate fits within the ceiling, keep the user's FPS
            // target. Capping to 30 unconditionally penalised fluency even on fast connections.
            if (streamBitrate == bitrateCeilingKbps && streamFps > 30) {
                streamFps = 30;
                LimeLog.info("Slow network profile: capping FPS to 30 for stability");
            }
        }
        ...
        StreamConfiguration config = new StreamConfiguration.Builder()
                ...
                .enable4GOptimizations(useNetworkOptimizations)
                .setJumpFrameMode(prefConfig.jumpFrameMode)
                .enableFrameSkip(useNetworkOptimizations && prefConfig.jumpFrameMode != StreamConfiguration.JUMPFRAME_MODE_OFF)
                .setTargetLatencyMs(prefConfig.targetLatencyMs)
                .setEnableAdaptiveResolution(useNetworkOptimizations && prefConfig.adaptiveResolution)
                .setBitrateOptimization(prefConfig.bitrateOptimization)
                .setBitrateAnalysisIntervalMs(prefConfig.bitrateAnalysisIntervalMs)
                .setFrameSimilarityThreshold(prefConfig.frameSimilarityThreshold)
                .setLocalUpscaling(prefConfig.localUpscaling)
                .setLocalUpscalingMode(prefConfig.localUpscalingMode)
                .setPreferAudioOverVideo(prefConfig.preferAudioOverVideo)
                .setTransitionFrameMode(prefConfig.transitionFrameMode)
                .setLocalFrameDeduplication(prefConfig.localFrameDeduplication)
                .setLocalMotionSmoothing(prefConfig.localMotionSmoothing)
                .build();
    ...
    private void stopConnection() {
        try {
            if (connecting || connected) {
                ...
                if (controllerHandler != null) {
                    try {
                        ...
                        LimeLog.warning("Erro ao parar controlador: " + e.getMessage());
                    }
                }
                ...
                try {
                    ...
                    LimeLog.warning("Erro ao notificar fim de stream: " + e.getMessage());
                }
                    ...
                    new Thread() {
                        public void run() {
                            try {
                                ...
                                LimeLog.warning("Erro ao parar conexão: " + e.getMessage());
                            }
                        }
                    }.start();
                }
            ...
            LimeLog.severe("Erro em stopConnection: " + e.getMessage());
            e.printStackTrace();
        }
    }
                ...
                try {
                    if (spinner != null) {
                        try {
                            ...
                            LimeLog.warning("Erro ao descartar spinner: " + e.getMessage());
                        }
                        spinner = null;
                    }
                    ...
                    if (!displayedFailureDialog) {
                        ...
                        if (stage.contains("video") && streamView != null && streamView.getHolder() != null && streamView.getHolder().getSurface() != null && streamView.getHolder().getSurface().isValid()) {
                    ...
                    LimeLog.severe("Erro em stageFailed: " + e.getMessage());
                    e.printStackTrace();
                }
            }
        });
            ...
            public void run() {
                try {
                    ...
                    if (!displayedFailureDialog) {
                        ...
                        // Reconexão automática: em erros recuperáveis de rede instável
                        // (típico de 4G/Wi-Fi ruim), tenta reconectar em vez de simplesmente
                        // jogar o usuário de volta para a tela de PCs.
                        boolean recoverableNetworkError =
                                errorCode == MoonBridge.ML_ERROR_NO_VIDEO_TRAFFIC ||
                                errorCode == MoonBridge.ML_ERROR_NO_VIDEO_FRAME ||
                                errorCode == MoonBridge.ML_ERROR_UNEXPECTED_EARLY_TERMINATION;

                        int reconnectAttempt = Game.this.getIntent().getIntExtra(EXTRA_RECONNECT_ATTEMPT, 0);
                        int maxReconnectAttempts = prefConfig.autoReconnect ? prefConfig.autoReconnectAttempts : 0;

                        if (recoverableNetworkError && reconnectAttempt < maxReconnectAttempts) {
                            final int nextAttempt = reconnectAttempt + 1;
                            Toast.makeText(Game.this,
                                    getResources().getString(R.string.reconnecting_msg) +
                                            " (" + nextAttempt + "/" + maxReconnectAttempts + ")",
                                    Toast.LENGTH_LONG).show();

                            final Intent reconnectIntent = new Intent(Game.this.getIntent());
                            reconnectIntent.putExtra(EXTRA_RECONNECT_ATTEMPT, nextAttempt);
                            ...
                            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        LimeLog.severe("Erro ao iniciar reconexão: " + e.getMessage());
                                    }
                                }
                            // Progressive delay: first retry is fast (800 ms) for brief 4G
                            // hiccups; subsequent retries back off to avoid hammering a truly
                            // broken link. Formula: 800 ms + 700 ms per previous attempt.
                            }, 800 + (nextAttempt - 1) * 700);
                            return;
                        }
                    }
                } catch (Exception e) {
                    LimeLog.severe("Erro em connectionTerminated: " + e.getMessage());
                    e.printStackTrace();
                }
